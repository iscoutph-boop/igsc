...
const CUSTOMER_EMAIL = 'v621-final-qa@example.com';
const QA_NAME = 'VMM V6.2.1 FINAL LIFECYCLE QA 20260901';
...
assert(health.version === '6.2.1-staging-self-service', `Expected V6.2.1, got ${health.version}`);

const create = await post('createBooking', {
  fullName: QA_NAME,
  phoneNumber: '09000006210',
  ...
  projectDetails: 'V6.2.1 FINAL STAGING LIFECYCLE QA ONLY — not a real customer lead.',
...
const reschedule = await post('rescheduleBooking', {
  ...
  newPreferredDate: '2026-09-08',
  newPreferredTime: '16:30',
  rescheduleNotes: 'V6.2.1 final staging reschedule QA.',
});
...
assert(String(find2.booking?.preferredDate || '').includes('2026-09-08'), `Expected date 2026-09-08, got ${find2.booking?.preferredDate}`);

const cancel = await post('cancelBooking', {
  bookingReference: reference,
  contact: CUSTOMER_EMAIL,
  cancellationReason: 'V6.2.1 final staging cancellation QA cleanup.',
});
...
console.log(`V621_QA_RESULT ${JSON.stringify({ reference, finalStatus: find3.booking.bookingStatus, version: health.version })}`);
