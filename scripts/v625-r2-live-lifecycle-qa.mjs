...
const customerEmail = `qa-r2-${stamp}@igsabroso.invalid`;
...
const createPayload = {
  submissionId,
  fullName: 'IGS V6.2.5 R2 FINAL STAGING QA',
  phoneNumber,
  ...
  projectDetails: `Controlled R2 lifecycle QA. Submission ${submissionId}`,
...
const rescheduled = await call('rescheduleBooking', {
  ...
  contact: phoneNumber,
  newPreferredDate: '2026-09-19',
  newPreferredTime: '14:30',
  rescheduleNotes: 'V6.2.5 R2 controlled staging QA reschedule',
});
...
const cancelled = await call('cancelBooking', {
  bookingReference: reference,
  contact: phoneNumber,
  cancellationReason: 'V6.2.5 R2 controlled staging QA cancellation',
});
