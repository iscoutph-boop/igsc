...
let json;
try { json = JSON.parse(text); } catch { throw new Error(`R2 health returned non-JSON: ${text.slice(0,300)}`); }
if (!response.ok) throw new Error(`R2 health HTTP ${response.status}: ${JSON.stringify(json)}`);
if (json.version !== expectedVersion) throw new Error(`Expected ${expectedVersion}, got ${json.version || 'missing version'}`);
